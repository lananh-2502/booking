...
export default function Home() {
  ...
  const reset = () => { setMode('home'); setGroup(null); setPersonName(''); setSelected([]); setAvatar(0); setNotice(''); loadGroups(); };
  ...
  return <main>
    ...
    <nav className="nav"><button className="brand" onClick={reset}><span>♡</span> Hẹn nha!</button><div className="nav-note"><i/>Rảnh lúc nào, chốt lúc đó</div></nav>
...
function AvatarPicker({ value, onChange }: { value: number; onChange: (value: number) => void }) { return <fieldset className="avatar-picker"><legend>Chọn avatar của m</legend><div>{AVATAR_NAMES.map((name, index) => <button type="button" className={value === index ? 'active' : ''} aria-label={name} aria-pressed={value === index} key={name} onClick={() => onChange(index)}><AvatarSticker avatar={index}/></button>)}</div><small>{AVATAR_NAMES[value]}</small></fieldset>; }
